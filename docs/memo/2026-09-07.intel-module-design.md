...
| P1 后是否做有效性检验 | **做** | P1 结束设硬 gate：50 篇手工标注比对，指标与阈值见 `2026-09-07.intel-module-plan.md` P1-Gate；不达标不进 P2 |

---

## 13. 与现有文档的关系

- `docs/memo/2026-09-04.architecture.md` §0「回测在 dc」——本次设计**不动**这条归口。intel 不实现回测。
- `docs/memo/2026-06-22.quant-closed-loop-roadmap.md` M3「技术面+另类因子扩展」——情报因子是该里程碑的落地形态之一。
- `docs/memo/2026-09-06.vibe-research-benchmark.md` —— 回测可信度那次借鉴；本文是第二次借鉴，取的是**数据纪律**而非回测。
