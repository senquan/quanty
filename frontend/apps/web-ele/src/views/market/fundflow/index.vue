...
const drawdownData = equityData.map((d, i) => {
  ...
  return { date: d.date, value: Number.parseFloat((((d.value - peak) / peak) * 100).toFixed(2)) };
});
  ...
  <div class="backtest-page p-4">
    ...
    <ElCard shadow="never" class="mb-4">
      <ElRow :gutter="12" align="middle">
        <ElCol>
          <ElSelect v-model="selectedStrategy" placeholder="选择策略" clearable style="width: 200px" class="mr-2">
            <ElOption label="双均线交叉策略" value="1" />
            <ElOption label="RSI均值回归策略" value="2" />
            <ElOption label="动量突破策略" value="3" />
          </ElSelect>
          <ElSelect placeholder="数据源" style="width: 160px" class="mr-2">
            <ElOption label="Yahoo Finance" value="yahoo" />
            <ElOption label="CCXT" value="ccxt" />
          </ElSelect>
          <ElInput class="mr-2" placeholder="标的代码" style="width: 140px" />
          <ElButton type="primary" @click="handleRunBacktest"><Play class="w-4 h-4 mr-1" />运行回测</ElButton>
        </ElCol>
      </ElRow>
    </ElCard>
