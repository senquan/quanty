...
- **WS auth**: the authoritative field is `STRATEGY_INTERNAL_TOKEN` — dc `.env` `STRATEGY_INTERNAL_TOKEN` must equal backend `.env` `STRATEGY_INTERNAL_TOKEN` (backend validates it in `app/ws/server.py:55`, and `main.py` warns on the same field). An empty value **disables** handshake verification. Note: `STRAT_INTEGRATION_TOKEN` is a legacy field the code does **not** read, and dc's `WS_TOKEN` does not exist at all (dead config).